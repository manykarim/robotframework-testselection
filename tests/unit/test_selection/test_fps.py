...
class TestFarthestPointSampling:
    def test_deterministic_with_seed(self, fps: FarthestPointSampling, random_vectors: np.ndarray) -> None:
        result1 = fps.select(random_vectors, k=5, seed=42)
        result2 = fps.select(random_vectors, k=5, seed=42)
        assert result1 == result2

    def test_k_equals_1_returns_single_index(self, fps: FarthestPointSampling, random_vectors: np.ndarray) -> None:
        result = fps.select(random_vectors, k=1, seed=42)
        assert len(result) == 1
        assert 0 <= result[0] < random_vectors.shape[0]

    def test_k_equals_n_returns_all_indices(self, fps: FarthestPointSampling, random_vectors: np.ndarray) -> None:
        n = random_vectors.shape[0]
        result = fps.select(random_vectors, k=n, seed=42)
        assert len(result) == n
        assert set(result) == set(range(n))

    def test_selected_indices_within_valid_range(self, fps: FarthestPointSampling, random_vectors: np.ndarray) -> None:
        result = fps.select(random_vectors, k=7, seed=42)
        assert len(result) == 7
        for idx in result:
            assert 0 <= idx < random_vectors.shape[0]

    def test_no_duplicate_indices(self, fps: FarthestPointSampling, random_vectors: np.ndarray) -> None:
        result = fps.select(random_vectors, k=10, seed=42)
        assert len(result) == len(set(result))

    def test_corner_geometry_selects_corners(self, fps: FarthestPointSampling, corner_vectors: np.ndarray) -> None:
